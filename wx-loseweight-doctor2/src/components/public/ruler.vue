                    ...
                    <template v-for="(item, index) in counter">
                        <li class="cs-scroll-item" :key="index">
                            <template v-if="index % 10 == 0">
                                <div
                                    class="cs-scroll-item-rule vux-1px-l cs-scale-integer"
                                ></div>
                                <div v-if="ispoint" class="cs-scroll-item-num">
                                    {{ (index + minNum) / 10 }}
                                </div>
                                <div v-else class="cs-scroll-item-num">
                                    {{ (index + minNum) * oneGridValue }}
                                </div>
                            </template>
                            <template v-else-if="index % 5 == 0">
                                <div
                                    class="cs-scroll-item-rule vux-1px-l cs-scale-half"
                                ></div>
                            </template>
                            <template v-else>
                                <div
                                    class="cs-scroll-item-rule vux-1px-l cs-scale-one"
                                ></div>
                            </template>
                        </li>
                    </template>
    ...
    methods: {
        ...
        init() {
            this.$nextTick(() => {
                ...
                this.scrollrule.scrollBy(
                    -(this.oneWidth * (this.NowNum - this.minNum)),
                    0,
                    0
                )
            })
        ...
        initScroll() {
            ...
            this.scrollrule.on('scroll', pos => {
                ...
                let NumValue =
                    Math.abs(Math.round(this.scrollX / this.oneWidth)) +
                    this.minNum
                // 判断是否开启小数
                if (this.ispoint) {
                    this.$emit(
                        'post-NumValue',
                        (NumValue / 10) * this.oneGridValue
                    )
                } else {
...
.setTopLine(@c: #C7C7C7) {
    ...
    right: 0;
    height: 1px;
    border-top: 1px solid rgba(180, 181, 217, 0.6);
    color: @c;
...
.setBottomLine(@c: #C7C7C7) {
    ...
    right: 0;
    height: 1px;
    border-bottom: 1px solid rgba(180, 181, 217, 0.6);
    color: @c;
    transform-origin: 0 100%;
    transform: scaleY(0.5);
}

.setLeftLine(@c: #B4B5D9) {
    content: ' ';
    position: absolute;
    left: 0;
    top: 0;
    width: 1px;
    bottom: 0;
    border-left: 1px solid rgba(180, 181, 217, 0.6);
    color: @c;
    transform-origin: 0 0;
    transform: scaleX(0.5);
}

.setRightLine(@c: #B4B5D9) {
    content: ' ';
    position: absolute;
    right: 0;
    top: 0;
    width: 1px;
    bottom: 0;
    border-right: 1px solid rgba(180, 181, 217, 0.6);
    color: @c;
...
.setLine(@c: #B4B5D9) {
    ...
    top: 0;
    width: 200%;
    border: 1px solid rgba(180, 181, 217, 0.6);
    color: @c;
    height: 200%;
...
.cs-scroll-item-pointer::before {
    ...
    height: 0;
    border-width: 6px;
    border-color: #6643e7 transparent transparent transparent;
