    ...
    <div v-show="showFlag" class="searchContent">
        <cube-scroll ref="scrollSearch">
            <div>
                ...
                <div
                    class="searchList"
                    v-show="patientsListSearch && patientsListSearch.length > 0"
                >
                    <div
...
export default {
    ...
    methods: {
        ...
        getJZMZPatientsSearch() {
            ...
            let doctorId = ''
            if (!filterInfo.doctorId) doctorId = this.$route.query.doctorId
            else doctorId = filterInfo.doctorId
            let checkedVal = filterInfo.checkedVal || ''
            let timeIndex = filterInfo.timeIndex || ''
            var _this = this
            let url = this.api.userApi.GetJZMZPatients
            let data = {
                PageIndex: this.page,
                PageSize: 10,
                Keyword: this.searchVal, //搜索关键词
                DoctorId: doctorId,
                UpdateTimeDay: timeIndex || '', //更新时间天数 7,15,30
                GroupId: timeIndex || '', // 分组
                QueryType: this.$route.query.type //查询类型 0 全部 1今日患者 2明日患者
            }
